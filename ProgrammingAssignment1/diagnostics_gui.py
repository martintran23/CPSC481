...
def on_selection():
    disease, p_disease =bn.diagnose(asia_var.get(), smoking_var.get(),
    xray_var.get(), dyspnea_var.get())
...
ttk.Label(root, text="Visit to Asia").grid(row=0, column=0, padx=10, pady=10,
sticky="w")
asia_dropdown = ttk.Combobox(root, textvariable=asia_var, values=["NA", "Yes",
"No"], state="readonly")
...
smoking_dropdown = ttk.Combobox(root, textvariable=smoking_var, values=["NA",
...
xray_dropdown = ttk.Combobox(root, textvariable=xray_var, values=["NA", "Abnormal",
...
dyspnea_dropdown = ttk.Combobox(root, textvariable=dyspnea_var, values=["NA",
"Present", "Absent"], state="readonly")
