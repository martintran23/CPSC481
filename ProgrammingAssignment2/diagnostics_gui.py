...
# Import the Project 2 Diagnostics class (lives in the same folder).
from diagnostics import Diagnostics


# Create the main application window
root = tk.Tk()
root.title("Medical Diagnostics (Project 2)")
...
def on_selection():
    # Read dropdown selections and run diagnosis.
    disease, p_disease = bn.diagnose(
        asia_var.get(),
        smoking_var.get(),
        xray_var.get(),
        dyspnea_var.get(),
...
ttk.Label(root, text="Visit to Asia").grid(
    row=0, column=0, padx=10, pady=10, sticky="w"
)
asia_dropdown = ttk.Combobox(
    root,
    textvariable=asia_var,
    values=["NA", "Yes", "No"],
    state="readonly",
)
asia_dropdown.grid(row=0, column=1, padx=10, pady=10)

ttk.Label(root, text="Smoking").grid(
...
ttk.Label(root, text="Xray").grid(
    row=2, column=0, padx=10, pady=10, sticky="w"
)
xray_dropdown = ttk.Combobox(
    root,
    textvariable=xray_var,
    values=["NA", "Abnormal", "Normal"],
    state="readonly",
)
xray_dropdown.grid(row=2, column=1, padx=10, pady=10)

ttk.Label(root, text="Dyspnea").grid(
    row=3, column=0, padx=10, pady=10, sticky="w"
)
dyspnea_dropdown = ttk.Combobox(
    root,
    textvariable=dyspnea_var,
    values=["NA", "Present", "Absent"],
    state="readonly",
)
